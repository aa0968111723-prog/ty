...
import {
  BarChart3, Check, ChevronDown, Flag, GripVertical,
  LayoutDashboard, ListFilter, LogOut, Medal, MoreVertical, Pin, PinOff, RefreshCw,
  RotateCcw, Settings, Sheet, Trophy, Users, X,
} from "lucide-react";
import { AdminLogin } from "@/components/admin-login";
import "@/admin.css";

export const Route = createFileRoute("/admin")({
  head: () => ({ meta: [{ title: "禪學社戰情｜淡江大學禪學社" }, { name: "robots", content: "noindex, nofollow" }] }),
...
type WidgetId =
  | "todayContacts" | "dateContacts" | "official" | "practice" | "highest" | "average"
  | "topThree" | "gatekeepers" | "departments" | "grades" | "latestForms"
  | "recentPlayers" | "sync" | "system";
...
function validIds(value: unknown): WidgetId[] {
  ...
  return [...new Set(value.filter((id): id is WidgetId => typeof id === "string" && IDS.includes(id as WidgetId)))];
}
...
function time(value: string) {
  ...
  return Number.isNaN(date.getTime()) ? "—" : date.toLocaleTimeString("zh-TW", { timeZone: "Asia/Taipei", hour: "2-digit", minute: "2-digit", hour12: false });
}
function initialView(): Tab {
  if (typeof window === "undefined") return "overview";
  const view = new URLSearchParams(window.location.search).get("view");
  return ({ today: "overview", ranking: "podium", contacts: "contacts", gatekeepers: "leaders", pinned: "pinned" } as Record<string, Tab>)[view || ""] ?? "overview";
}
...
function Podium({ rows, compact = false }: { rows: Result[]; compact?: boolean }) {
  return !rows.length ? <p className="admin-empty">尚無正式挑戰紀錄</p> : <ol className={`admin-podium${compact ? " is-compact" : ""}`}>{rows.map((row, index) => (
...
function Kpi({ label, value, hint }: { label: string; value: number | string; hint?: string }) {
  return <div className="admin-widget-kpi"><span>{label}</span><strong>{typeof value === "number" ? value.toLocaleString() : value}</strong>{hint && <small>{hint}</small>}</div>;
}
...
function AdminDashboard() {
  ...
  const [source, setSource] = useState(() => typeof window !== "undefined" && new URLSearchParams(window.location.search).get("view") === "form" ? "Google Form" : "");
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const load = async (target: string) => {
        const response = await fetch(`/api/admin/dashboard?date=${encodeURIComponent(target)}`, { cache: "no-store", signal: AbortSignal.timeout(15000) });
  ...
  function setView(next: Tab, shortcut?: string) {
    ...
    if (shortcut) url.searchParams.set("view", shortcut); else url.searchParams.delete("view");
  ...
  function toggleVisible(id: WidgetId) {
    setLayout((current) => {
      const visible = current.visible.includes(id) ? current.visible.filter((item) => item !== id) : [...current.visible, id];
      return { ...current, visible, pinned: current.pinned.filter((item) => visible.includes(item)) };
    });
  }
  function togglePinned(id: WidgetId) {
    setLayout((current) => {
      ...
      return { ...current, pinned, visible: current.visible.includes(id) ? current.visible : [...current.visible, id] };
    });
  }
  ...
  const orderedVisible = useMemo(() => layout.order.filter((id) => layout.visible.includes(id)), [layout]);
  ...
  function widget(id: WidgetId, editor = false): ReactNode {
    ...
    let body: ReactNode;
    switch (id) {
      case "todayContacts": body = <Kpi label="今日接觸" value={todayData?.kpis.contacts ?? "—"} hint="去重複人數" />; break;
      case "dateContacts": body = <Kpi label={date === taipeiDate() ? "今日接觸" : "當日接觸"} value={data.kpis.contacts} hint={date.replaceAll("-", " / ")} />; break;
      case "official": body = <Kpi label="正式挑戰" value={data.kpis.officialChallenges} hint="完成紀錄" />; break;
      case "practice": body = <Kpi label="試玩接觸" value={practice} hint="接觸扣除正式挑戰" />; break;
      case "highest": body = <Kpi label="最高分" value={data.kpis.highestScore} hint="正式挑戰" />; break;
      case "average": body = <Kpi label="平均分數" value={data.kpis.averageScore} hint="正式挑戰" />; break;
      case "topThree": body = <><h2><Medal size={20} />前三名</h2><Podium rows={data.topThree} compact /></>; break;
      case "gatekeepers": body = <><h2>關主統計</h2><Bars rows={data.gatekeepers} onSelect={selectLeader} /></>; break;
      case "departments": body = <><h2>科系分布</h2><Bars rows={data.departments} /></>; break;
      case "grades": body = <><h2>年級分布</h2><Bars rows={data.grades} /></>; break;
      case "latestForms": body = <><h2><Sheet size={19} />Google 表單最新資料</h2>{latestForms.length ? <ul className="admin-mini-list">{latestForms.map((row, index) => <li key={`${row.completedAt}-${index}`}><strong>{row.name}</strong><span>{time(row.completedAt)}</span></li>)}</ul> : <p className="admin-empty">尚無表單資料</p>}</>; break;
      case "recentPlayers": body = <><h2><Trophy size={19} />最近參賽者</h2>{data.results.length ? <ul className="admin-mini-list">{[...data.results].sort((a, b) => b.completedAt.localeCompare(a.completedAt)).slice(0, 3).map((row) => <li key={row.id}><strong>{row.name}</strong><span>{row.score.toLocaleString()} 分</span></li>)}</ul> : <p className="admin-empty">尚無參賽紀錄</p>}</>; break;
      case "sync": body = <Kpi label="最後同步" value={time(data.sync.updatedAt)} hint={data.sync.forms.ok && data.sync.results.ok ? "Google Sheet 已連線" : "部分同步異常"} />; break;
      case "system": body = <><h2>系統狀態</h2><dl className="admin-system compact"><dt>表單</dt><dd>{data.sync.forms.ok ? "正常" : "異常"}</dd><dt>成績</dt><dd>{data.sync.results.ok ? "正常" : "異常"}</dd><dt>自動更新</dt><dd>30 秒</dd></dl></>; break;
    }
    return <article
      key={id}
      ...
      onDrop={() => { if (dragging) moveWidget(dragging, id); setDragging(null); }}
    >
      {editor && <div className="admin-widget-tools">
        <MoreVertical size={18} aria-hidden="true" />
        <button onClick={() => togglePinned(id)} aria-label={`${layout.pinned.includes(id) ? "取消釘選" : "釘選"}${WIDGETS.find((item) => item.id === id)?.label}`}>
          {layout.pinned.includes(id) ? <PinOff size={18} /> : <Pin size={18} />}
        </button>
        <button onClick={() => toggleVisible(id)} aria-label={`${layout.visible.includes(id) ? "隱藏" : "顯示"}${WIDGETS.find((item) => item.id === id)?.label}`}>
          {layout.visible.includes(id) ? <X size={18} /> : <Check size={18} />}
        </button>
        <span className="admin-drag-handle" aria-label="拖曳排序"><GripVertical size={20} /></span>
      </div>}
      {body}
      {editor && !layout.visible.includes(id) && <span className="admin-hidden-label">已隱藏</span>}
    </article>;
  }
  ...
  return (
    <main className="admin-page">
      <aside className="admin-nav">
        ...
        <nav aria-label="後台導覽">{tabs.map(({ id, label, icon: Icon }) => <button key={id} aria-current={tab === id ? "page" : undefined} onClick={() => setView(id)}><Icon size={21} /><span>{label}</span></button>)}</nav>
        <button className="admin-logout" onClick={logout}><LogOut size={18} />登出</button>
      </aside>
      <div className="admin-content">
        <header className="admin-heading"><div><span className="admin-eyebrow">淡江大學禪學社 · 工作人員專用</span><h1>{tab === "pinned" ? "我的戰情" : "社博即時戰情"}</h1><p>{date.replaceAll("-", " / ")}</p></div>
          <div className="admin-heading-actions">
            {(tab === "overview" || tab === "pinned") && <button className={editing ? "admin-done" : "admin-customize"} onClick={() => setEditing((value) => !value)}>{editing ? <Check size={19} /> : <LayoutDashboard size={19} />}<span>{editing ? "完成" : "自訂"}</span></button>}
        ...
        {data && (tab === "overview" || tab === "pinned") && <>
          {editing ? <section className="admin-editor" aria-label="自訂儀表板">
            <div className="admin-editor-heading"><div><h2>自訂儀表板</h2><p>拖曳卡片排序，並選擇要顯示或釘選的資訊。</p></div>
              <button onClick={() => setLayout(DEFAULT_LAYOUT)}><RotateCcw size={17} />恢復預設</button>
            </div>
            <div className="admin-widget-grid is-editing">{layout.order.map((id) => widget(id, true))}</div>
          </section> : <>
            <section className="admin-widget-grid admin-pinned-grid" aria-label={tab === "pinned" ? "我的戰情" : "釘選資訊"}>
              {pinned.map((id) => widget(id))}
              {!pinned.length && <div className="admin-panel admin-empty">尚未釘選卡片，點「自訂」開始設定。</div>}
            </section>
            {tab === "overview" && more.length > 0 && <section className="admin-more">
              <button className="admin-more-toggle" aria-expanded={showMore} onClick={() => setShowMore((value) => !value)}>
                <span><ListFilter size={19} />更多資訊</span><ChevronDown size={20} className={showMore ? "is-open" : ""} />
              </button>
              {showMore && <div className="admin-widget-grid">{more.map((id) => widget(id))}</div>}
            </section>}
            <section className="admin-quick" aria-label="快速入口">
              <h2>快速入口</h2><div>
                {[
                  ["名單", Users, "contacts", "contacts"], ["前三名", Medal, "podium", "ranking"],
                  ["關主", Flag, "leaders", "gatekeepers"], ["表單", Sheet, "contacts", "form"],
                  ["成績", Trophy, "results", "results"], ["同步", RefreshCw, "system", "sync"],
                ].map(([label, Icon, next, shortcut]) => <button key={label as string} onClick={() => { setSource(shortcut === "form" ? "Google Form" : ""); setView(next as Tab, shortcut as string); }}><Icon size={21} /><span>{label as string}</span></button>)}
              </div>
            </section>
          </>}
        </>}

        {data && tab === "podium" && <section className="admin-panel"><h2><Medal size={20} /> {date === taipeiDate() ? "今日" : "當日"}前三名</h2><p className="admin-caption">僅正式挑戰 · 指定日期 · 排名不公開</p><Podium rows={data.topThree} /></section>}
        {(tab === "contacts" || tab === "results") && <section className="admin-panel">
          <div className="admin-section-heading"><h2>{tab === "contacts" ? "聯絡名單" : "比賽成績"}</h2>{tab === "results" && <button onClick={() => setView("podium", "ranking")}>查看前三名</button>}</div>
