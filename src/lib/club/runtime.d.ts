...
export const SPEED_PRESETS: { id: string; label: string; hint: string; switchMs: number; comboEvery: number; tapLockMs: number }[];
export const DURATION_MIN: number;
export const DURATION_MAX: number;
export const CLUB_NAME: string;
export const COLORS: { id: "red" | "blue" | "green" | "yellow"; label: string; hex: string; key: string[] }[];
...
export function scoreIsConsistent(row: { score: number; correct: number; wrong: number; maxCombo: number }): boolean;
...
export function nextQuestion(prev: unknown): { meaning: { id: string; label: string; hex: string }; visual: { id: string; label: string; hex: string } };
export function colorByKey(key: string): string | null;
export function correctId(game: LiveGame): string;
export function createLiveGame(now?: number, opts?: { skipSave?: boolean; settings?: unknown }): LiveGame;
...
export function judgeAnswer(
  ...
  now?: number,
): { ok: boolean; hit?: boolean; reason?: string; expected?: string; score?: number; combo?: number; delta?: number };
export function publicResult(game: LiveGame, player: { name: string; department: string; grade: string; phone: string }): Record<string, unknown> & {
  name: string;
  score: number;
  correct: number;
  wrong: number;
...
export function tickGame(game: LiveGame, now?: number): { remaining: number; switched: boolean; expired: boolean };
export function validatePlayer(player: unknown):
  | { ok: true; errors: Record<string, string | undefined>; data: { name: string; department: string; grade: string; phone: string } }
...
export type LiveGame = {
  ...
  question: { meaning: { id: string; label: string; hex: string }; visual: { id: string; label: string; hex: string } };
};
