...
正式站還需要：`GOOGLE_SERVICE_ACCOUNT_JSON`、`GOOGLE_SHEET_ID`、`ADMIN_SESSION_SECRET`、`PUBLIC_ORIGIN`。管理員登入另需 `GOOGLE_OAUTH_CLIENT_ID`、`GOOGLE_OAUTH_CLIENT_SECRET`、`ADMIN_ALLOWED_EMAILS`。`ADMIN_PASSWORD` 僅為隱藏緊急備用。不要使用 `VITE_` 前綴。
