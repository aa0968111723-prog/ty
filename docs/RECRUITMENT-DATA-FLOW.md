...
夥伴在表單裡繼續填活動、入社、保證金、備註其餘內容。預填欄位都可改。後台內建填表改問「這位同學報名了哪個活動？」，不再顯示分級／S／A／B。
...
已登入夥伴可在 `/follow-up` 直接填完正式表單其餘題目（活動、入社、保證金、備註、入社後的生日／學號／興趣），按「送出招生資料」。伺服器以管理員 session（`protect`／同源 POST）呼叫 Sheets API，**append** 一列到「招生狀況表」（gid `1921679351`），欄位對應 Google Form 回應列。技術欄 `_gameSubmissionId`、`_gameGatekeeper`、`_gameCompletedAt` 只加在最後，不刪既有欄。去重與表單提交相同：同一 `submissionId` 或同一正規化電話不重複寫入，並立刻從待跟進名單移除。
...
正式站還需要：`GOOGLE_SERVICE_ACCOUNT_JSON`、`GOOGLE_SHEET_ID`、`ADMIN_PASSWORD`、`ADMIN_SESSION_SECRET`、`PUBLIC_ORIGIN`。未設定管理員密碼與 session secret 前，現場仍可用攤位密碼登入。可選 `GOOGLE_OAUTH_CLIENT_ID`、`GOOGLE_OAUTH_CLIENT_SECRET`、`ADMIN_ALLOWED_EMAILS` 作為額外 Google 登入。不要使用 `VITE_` 前綴。
