...
test(
  ...
  async (t) => {
    ...
    await t.test("queue defaults to people related to the selected recruiter", async () => {
      ...
      const mineDetail = page.getByRole("article").filter({ hasText: "關主的同學" }).getByRole("button", { name: "查看詳細" });
      const mineDetailBox = await mineDetail.boundingBox();
      const queueNav = await page.getByRole("navigation", { name: "手機後台導覽" }).boundingBox();
      assert.ok(
        mineDetailBox && queueNav && mineDetailBox.y + mineDetailBox.height <= queueNav.y + 1,
        `查看詳細 behind nav detail=${JSON.stringify(mineDetailBox)} nav=${JSON.stringify(queueNav)}`,
      );
    ...
    await t.test("roster date filters cover 今日 昨日 指定日期 歷史全部", async () => {
      const context = await browser.newContext({ viewport: { width: 390, height: 844 }, isMobile: true, hasTouch: true });
      const page = await context.newPage();
      const errors = [];
      page.on("pageerror", (error) => errors.push(error.message));
      ...
      const today = {
        姓名: "今日甲", 電話: "0910000001", 科系: "歷史學系", 年級: "大一", 遊戲關主: "柏能",
        _submissionId: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa", _kind: "official", _skipSave: false,
        遊戲時間: "2026-09-14T01:00:00.000Z",
      };
      const yesterday = {
        姓名: "昨日乙", 電話: "0910000002", 科系: "會計學系", 年級: "大二", 遊戲關主: "柏能",
        _submissionId: "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb", _kind: "official", _skipSave: false,
        遊戲時間: "2026-09-13T01:00:00.000Z",
      };
      const older = {
        姓名: "指定丙", 電話: "0910000003", 科系: "資訊工程學系", 年級: "大三", 遊戲關主: "柏能",
        _submissionId: "cccccccc-cccc-4ccc-8ccc-cccccccccccc", _kind: "official", _skipSave: false,
        遊戲時間: "2026-09-10T01:00:00.000Z",
      };
      await page.route("**/api/admin/recruitment**", (route) => route.fulfill({
        json: buildRecruitmentDashboard({
          date: "2026-09-14",
          gameRows: [today, yesterday, older],
          recruitmentRows: [],
          masterRows: [],
        }),
      }));
      ...
      await page.getByRole("button", { name: "展開篩選" }).waitFor();
      assert.equal(await page.getByLabel("日期範圍").isVisible(), false);
      assert.equal(await page.getByRole("article").filter({ hasText: "今日甲" }).count(), 1);
      assert.equal(await page.getByRole("article").filter({ hasText: "昨日乙" }).count(), 1);
      assert.equal(await page.getByRole("article").filter({ hasText: "指定丙" }).count(), 1);
      assert.ok(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1));
      await page.getByRole("button", { name: "展開篩選" }).click();
      const range = page.getByLabel("日期範圍");
      await range.waitFor();
      assert.deepEqual(await range.locator("option").allTextContents(), ["今日", "昨日", "指定日期", "歷史全部"]);
      await range.selectOption("today");
      assert.equal(await page.getByRole("article").filter({ hasText: "今日甲" }).count(), 1);
      assert.equal(await page.getByRole("article").filter({ hasText: "昨日乙" }).count(), 0);
      assert.equal(await page.getByRole("article").filter({ hasText: "指定丙" }).count(), 0);
      await range.selectOption("yesterday");
      assert.equal(await page.getByRole("article").filter({ hasText: "昨日乙" }).count(), 1);
      assert.equal(await page.getByRole("article").filter({ hasText: "今日甲" }).count(), 0);
      await range.selectOption("date");
      const picked = page.getByLabel("指定日期");
      await picked.waitFor();
      assert.equal(await picked.inputValue(), "2026-09-14");
      assert.equal(await page.getByRole("article").filter({ hasText: "今日甲" }).count(), 1);
      assert.equal(await page.getByRole("article").filter({ hasText: "指定丙" }).count(), 0);
      await picked.fill("2026-09-10");
      assert.equal(await page.getByRole("article").filter({ hasText: "指定丙" }).count(), 1);
      assert.equal(await page.getByRole("article").filter({ hasText: "今日甲" }).count(), 0);
      assert.equal(await page.getByRole("article").filter({ hasText: "昨日乙" }).count(), 0);
      await capture(page, "roster-dates-custom-390");
