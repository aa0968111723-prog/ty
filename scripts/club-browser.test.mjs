...
test(
  "club mobile DOM, scrolling, admin filters and game interactions (no screenshots)",
  { skip: !base },
  async (t) => {
    ...
    for (const [width, height] of [
      ...
      await t.test(`${width}x${height} registration and dashboard scroll`, async () => {
        ...
        await page.getByRole("button", { name: "完成", exact: true }).click();
        await page
          .getByRole("navigation", { name: "後台導覽" })
        ...
        await page.getByRole("heading", { name: "我的戰情" }).waitFor();
