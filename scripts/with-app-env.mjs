...
function main(argv) {
  ...
  // `spawn` does not resolve npm's `.cmd` shims on Windows when shell mode is
  // disabled. Keep the safer direct-spawn behavior while making npm scripts
  // work from both Windows and Unix environments.
